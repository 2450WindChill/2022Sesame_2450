...
public class MoveArmCommand extends CommandBase {
  ...
  public MoveArmCommand(double SetPoint, ClimberSubsystem m_Climb) {
    m_subsystem = m_Climb;
    m_SetPoint = SetPoint;
...
// If the arm distance is equal to the setpoint stop motors
  ...
  public boolean isFinished() {
    boolean limitSwitchValue = m_subsystem.lernieUp.get();
        ...
        if ((limitSwitchValue == true) || ((m_SetPoint < m_subsystem.encoder1.getDistance() - offset)) && (m_SetPoint > m_subsystem.encoder1.getDistance() + offset)) {
            return true;
        }
        return false;
    }
